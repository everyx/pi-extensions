...
- **错误分层**：LLM 可见文本是简洁错误，配置指引（装什么 key、怎么配）走 `details`（UI 可见），不进 LLM 上下文。**且必须落在渲染器真会读的键上**——本仓卡片渲染的文字通道是 `details.error`（`pi-ui/view.ts` 只把 `details.data` + `details.error` 交给视图），写进没人读的字段等于写给空气。
