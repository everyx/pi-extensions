...
- **Foreground** — the sub‑agent's output streams live into the tool card, word by word (rpc `text_delta` events forwarded to `onUpdate`). Header shows the quoted task title in the bash-command color: `Agent "research db schema" (model)`.
- **Background spawn** — a single status line confirms the spawn (no header, the title lives on the line): `⠋ Agent "probe" · starting…` → `✓ Agent "probe" · started` (or `✗ Agent "probe" · start failed` with the reason on a dim second line). The leading icon (spinner / ✓ / ✗) makes the state readable at a glance. The agent id stays in the tool content for the LLM's AgentControl — never shown on the card.
