...
- **用户（TUI）**：看到完整诊断（原因 + 建议），展开可见全部细节（对齐统一视觉语法的折叠预览）。配置指引（如 `Set FIRECRAWL_API_KEY…`）经 `summarizeFailures` 拼进 `details.error`——那是卡片唯一渲染的文字通道；失败记录里的 `hint` 字段本身无渲染器读，单独放着等于没写。
