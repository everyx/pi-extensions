...
- **Auto fallback** (non-office files read as raw utf-8 text): text PDF locally → `needsOcr` scanned pages → `hosted` (Firecrawl Parse, 2 qps, 1k pages/month — charged per OCR'd page, local-month reset, at `~/.pi/agent/pi-read-doc.json`) → `rapidocr` (local `python-rapidocr`, `pdf` only).
